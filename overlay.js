	...
	$.fn.extend({
		overlay: function(options) {
			...
			return this.each(function() {
				...
				// 指定了关闭按钮
				if(options.closeEle){
					$(document).on('click',options.closeEle,function(e){
						e.preventDefault();
						removeOverlay();
					})
				}
				...
				function showOverlay() {
					if ($('#cover').length > 0) {
						...
						}).show(); //adding new background and opacity if it already exists
					} else {
				...
				function fixMessage() {
					if (options.center) { // 如果为居中定位
						...
						if (remainingHeight < 40) {
							...
							$this.css({
								'position': o.position,
								'top': remainingHeight / 2 + 'px'
							});
						} else {
							$this.css({
								'position': o.position,
								'top': (remainingHeight / 2) + $(window).scrollTop() + 'px'
							});
						}
